...
impl Location {
  ...
  pub fn with_source(source_text: &str, offset: usize, len: usize) -> Self {
    let start = Position::with_source(source_text, offset);
    let end = Position::with_source(source_text, offset + len);
    Self { start, end }
  }
}
